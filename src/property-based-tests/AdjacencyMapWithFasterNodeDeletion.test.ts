...
describe('properties', () => {
    function syncConcurrently(yDocs: Y.Doc[], yGraphs: AdjacencyMapWithFasterNodeDeletion[]) {
    ...
    function runTest(clientCount: number, initialGraphSize: number, maxGraphSize: number, maxOperationsPerRoundCount: number) {
        fc.assert(
            fc.property(
                ...
                const yGraphs = yDocs.map((yDoc) => new AdjacencyMapWithFasterNodeDeletion(yDoc.getMap('adjacency map')))
